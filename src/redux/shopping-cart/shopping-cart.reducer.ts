import * as cartActions from "./shopping-cart.action";
...
export const reducer = (state = initialState, action : cartActions.ActionType) : cartState => {
    switch (action.type){
        case (cartActions.INCR_PRODUCT_QTY) : {
            let incProduct : IProduct[] = state.product.map((product) => {
                if(action.payload.id === product.sno){
                    return {
                        ...product,
                        qty : product.qty +1
        ...
        case (cartActions.DECR_PRODUCT_QTY) : {
            let decProduct : IProduct[] = state.product.map((product) => {
                if(action.payload.id === product.sno){
                    return {
                        ...product,
